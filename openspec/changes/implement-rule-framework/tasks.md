...
- [x] 0.1 Confirm `implement-analysis-framework`'s own 52 tasks are all
...
- [x] 1.1 Add the `aip-rules` Maven module (Java 21+) as a sibling of
  `aip-csm-builder` and `aip-analysis`, per `define-rule-framework/
  design.md` Decision 7's dependency diagram. Depends on: 0.1.
- [x] 1.2 Configure `aip-rules`'s POM to depend on `aip-core` only —
...
- [x] 2.2 Add tests confirming Rule Scope instances constructed via
...
- [x] 3.1 Define the `AnalysisResultSource` contract in `aip-core`
...
- [x] 3.2 Add contract tests for `AnalysisResultSource` independent of
...
- [x] 4.1 Implement `aip-rules` read access to `AnalysisView` for
  ...
  Analysis Content Reached Only Through Established Contracts`).
  Depends on: 1.2, 2.1.
- [x] 4.2 Implement retrieval of the source `CsmSnapshotId` from the
...
- [x] 4.3 Add a compile-time/dependency-graph test proving `aip-rules`
...
- [x] 5.1 Define the Rule Type contract (`aip-rules`): stable
  ...
  Depends on: 2.1.
- [x] 5.2 Enforce that a Rule Type is evaluable using exactly its
...
- [x] 5.3 Add tests confirming a Rule Type's declared inputs are static
...
- [x] 6.1 Define the Rule configuration shape (`aip-rules`-local, no
...
- [x] 6.2 Implement independent evaluation of multiple Rules configured
...
- [x] 6.3 Enforce that no Rule consumes another Rule's Rule Evaluation
  Result, and that Rule Framework accepts no declared ordering or
  dependency between Rules (spec `No Rule-to-Rule Composition`).
  Depends on: 6.1.
- [x] 6.4 Add tests confirming a Rule Evaluation Result is unaffected by
...
- [x] 7.1 Implement a Rule Type registry (registration API,
  identifier-based lookup), mirroring `AnalyzerRegistry`'s shape (spec
  `Extension Mechanism for New Rule Types`). Depends on: 5.1.
- [x] 7.2 Implement Rule registration against a registered Rule Type,
...
- [x] 7.3 Confirm registering a new Rule Type requires no change to
...
- [x] 8.1 Implement Rule Scope instance enumeration via `CsmScope`:
...
- [x] 8.2 Implement kind-based applicability: a Rule Scope instance
...
- [x] 8.3 Implement native-attribute applicability refinement as an
...
- [x] 8.4 Add tests for both applicability layers: no matching kind →
  ...
  Depends on: 8.2, 8.3.
- [x] 8.5 Enforce single-repository evaluation scope: every Rule
...
- [x] 9.1 Implement Rule Type consumption of its declared Analyzer
...
- [x] 9.2 Add a dependency-graph test proving a Rule Type never reads
...
- [x] 9.3 Add tests confirming a Rule Type evaluates identically against
...
- [x] 10.1 Confirm and enforce that all CSM content a Rule Type reads
...
- [x] 10.2 Add tests covering both established-contract boundaries
...
- [x] 11.1 Implement Rule evaluation as a pure function of a Rule's
...
- [x] 11.2 Add repeatability tests: evaluating the same Rule twice
...
- [x] 12.1 Define `RuleEvaluationOutcome` as a three-value `aip-core`
  enum (`PASS`, `FAIL`, `NOT_APPLICABLE`), per this change's own
  `design.md` Decision 4. Depends on: 0.1, 11.1.
- [x] 12.2 Implement the binding distinction (`define-rule-framework/
...
- [x] 12.3 Implement that every outcome — `PASS`, `FAIL`, or
...
- [x] 12.4 Add tests covering all three outcomes plus the no-Result
...
- [x] 13.1 Define `RuleEvaluationResult` as a final, non-generic class
...
- [x] 13.2 Define `RuleEvaluationResultId` in `aip-core`: a
  ...
  Depends on: 13.1.
- [x] 13.3 Add identity-stability tests: identical inputs across two
...
- [x] 13.4 Add identity-distinguishability tests: a differing consumed
...
- [x] 14.1 Implement traceability-reference retention on every
...
- [x] 14.2 Add tests confirming a `RuleEvaluationResult` is traceable
...
- [x] 15.1 Implement `RuleEvaluationResult`'s `Object` payload as an
...
- [x] 15.2 Add tests confirming two Rule Types' differently-shaped
...
- [x] 16.1 Define the `RuleEvaluationResultStore` abstraction
...
- [x] 16.2 Add tests confirming individual retrievability by identity
...
- [x] 17.1 Implement `RuleEvaluationResultValidator`, checking:
...
- [x] 17.2 Implement `RuleEvaluationResultPublisher`, calling
  `RuleEvaluationResultStore.write` only for a Result that passes
  validation. Depends on: 17.1, 16.1.
- [x] 17.3 Add tests for all four validation scenarios: valid Result
...
- [x] 18.1 Implement a test-only fixture-building API for
...
- [x] 18.2 Add a build/lint check confirming the fixture package is
...
- [x] 19.1 Add a test confirming `RuleEvaluationResult`s are never
  ...
  Depends on: 16.1.
- [x] 19.2 Add a test confirming a CSM-content or `AnalysisResult`
...
- [x] 19.3 Add a full-module dependency-graph test asserting
  `aip-rules`, `aip-csm-builder`, and `aip-analysis` remain siblings —
  none depends on either of the other two. Depends on: 1.3.
- [x] 19.4 Add an invariant test enumerating every requirement in this
...
- [x] 20.1 Implement a second, structurally distinct example Rule Type
...
- [x] 20.2 Add a test proving registering this second Rule Type
...
- [x] 21.1 Build a traceability matrix mapping each of `define-rule-
...
- [x] 21.2 Confirm every one of `define-rule-framework/design.md`'s
...
- [x] 21.3 Add an end-to-end fixture-based test evaluating a
  ...
  17.2, 18.1.
- [x] 21.4 Run full validation (`openspec validate rule-framework
...
- [x] 22.1 Finalize the dependency-graph CI check enforcing `aip-rules`
  depends on `aip-core` only — never `aip-analysis`, `aip-csm-builder`,
  or `aip-analyzer`. Depends on: 1.3.
- [x] 22.2 Add a source-scan check confirming no code path in
  ...
  on: 18.1.
- [x] 22.3 Final gate: confirm (via review checklist and CI) that this
